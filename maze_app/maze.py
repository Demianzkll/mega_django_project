...
class Maze:
    def __init__(self, size):
        ...
        self.grid = [[0 for col in range(self.size)] for row in range(self.size)]
        self.path = []
        self.visited_order = []   # порядок обходу DFS
    ...
    def solve(self):
        ...
        while stack:
            ...
            if node.loc in visited:
                continue

            visited.add(node.loc)
            self.visited_order.append(node.loc)
            ...
            if node.loc == (self.size-1, self.size-1):
                ...
                while tn is not None:
                    self.path.append(tn.loc)
                    tn = tn.parent
                self.path.reverse()  # від старту до фінішу
                return
        ...
        # якщо шляху немає, path залишиться пустим
